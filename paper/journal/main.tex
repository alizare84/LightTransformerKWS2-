...
pruning confirms this concern: test accuracy improves by
$+0.43\pm0.21$ percentage points on average across five seeds
purely from continued training
(from $94.12\pm0.32\%$ to $94.55\pm0.22\%$;
see \texttt{results/t\_ft\_control.csv}).
The pruning fine-tune gain of approximately $+0.43$ pp is therefore
consistent with being largely a training-duration effect rather than
a structural benefit of pruning itself.
...
The T$+$2{,}000-step fine-tuning control is also complete across
five seeds (mean gain $+0.43\pm0.21$ pp; \texttt{results/t\_ft\_control.csv}).
A parameter-matched
untied depth-1 control (U-L1; 36{,}516 unique parameters), a tied-MHA
control ($G{=}H{=}8$), and seed-0 position ablations remain queued under the
